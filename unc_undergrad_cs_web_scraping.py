...
# Base URL for BS program
BASE_URL = "https://cs.unc.edu/undergraduate/degrees/"
visited_links = set()
data = []
...
def crawl_and_scrape(start_url):
    """Crawls and scrapes all BS pages while removing empty content links"""
    driver = setup_driver()
    to_visit = {start_url}
    ...
    while to_visit:
        ...
        to_visit.update(new_links - visited_links)  # Add new links
    ...
    with open("unc_bs_program_cleaned.json", "w", encoding="utf-8") as json_file:
        json.dump(cleaned_data, json_file, indent=4, ensure_ascii=False)

    print("\n✅ Finished scraping all undergraduate pages! Data saved to 'unc_bs_program_cleaned.json'.")

# Start crawling at BS program page
